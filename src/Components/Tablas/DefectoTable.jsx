...
import DefectoModal from "../Modales/DefectoModal";

const DefectoTable = ({search, reloadFlag, onRefresh}) =>{
    ...
    const filtered = defectos.filter(item =>
    ...
    const deleteDefecto = (data) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                title: "Eliminado!",
                text: `El defecto ${data.defecto} ha sido eliminado`,
                icon: "success"
                });
                onRefresh()
            }
    ...
    return(
        <>
            {
                modalEdit && <DefectoModal modal={setModalEdit} defecto={defecto} isEdit={ismodal} onRefresh={onRefresh}/>
            }
            <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={() => {newDefecto()}} className="group flex items-center justify-start w-11 h-11 bg-green-600 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-32 hover:rounded-lg active:translate-x-1 active:translate-y-1">
                    ...
                    {paginatedData.map((item, i) => (
                        ...
                        <TableCell>
                            <div className="flex items-center justify-center gap-x-2">
                                <button onClick={() => {editDefecto(item)}} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-orange-400 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                ...
                                <button onClick={() => {deleteDefecto(item)}} className="group relative flex items-center justify-start w-fit py-1 px-3 bg-red-600 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                    <div className="flex items-center space-x-1 text-white text-xs font-semibold">
