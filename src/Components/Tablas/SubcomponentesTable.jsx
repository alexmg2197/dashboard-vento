...
import { Table, Box, TableHead, Typography, Pagination, TableRow, TableCell, TableBody, TableContainer, Paper } from "@mui/material";
import Swal from "sweetalert2";
import { supabase } from "../../../supabaseClient";
import LogoS from '../../assets/logo-vento.png'
import SubcomponenteModal from "../Modales/SubcomponenteModal";

const SubcomponentesTable = ({ search, componenteSeleccionado, reloadFlag, onRefresh }) => {
    ...
    const filtered = subcomponentes.filter(d =>
        d.Componentes.componente.toLowerCase().includes(search.toLowerCase()) ||
        d.subcomponente.toLowerCase().includes(search.toLowerCase())
    );
    ...
    const newSubcomponente = () =>{
        setSubcomponente(null);
        setIsModal(false)
        setModalEdit(true);
    }
    ...
    const deleteSubcomponente = (data) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .from('Subcomponentes')
                .delete()
                .eq('id_subcomponente',data.id_subcomponente)
                if(error){
                    Swal.fire({
                        title:`Error: ${error}`
                    })
                ...
                Swal.fire({
                title: "Eliminado!",
                text: `El subcomponente ${data.subcomponente} ha sido eliminado`,
                icon: "success"
                });
                onRefresh()
            }
    ...
    return (
        <>
            {
                modalEdit && <SubcomponenteModal modal={setModalEdit} subcomponente={subcomponente} isEdit={ismodal} onRefresh={onRefresh}/>
            }
            <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={() => {newSubcomponente()}} className="group flex items-center justify-start w-11 h-11 bg-green-600 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-52 hover:rounded-lg active:translate-x-1 active:translate-y-1">
            ...
            <TableContainer component={Paper}>
                <Table>
                    ...
                    <TableRow>
                        ...
                        <TableCell>Subcomponente</TableCell>
                    ...
                    {filtered.map((item, i) => (
                        ...
                        <TableCell>{item.subcomponente}</TableCell>
                        <TableCell>
                            <div className="flex items-center justify-center gap-x-2">
                                <button onClick={() => {editSubcomponente(item)}} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-orange-400 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                ...
                                <button onClick={() => {deleteSubcomponente(item)}} className="group relative flex items-center justify-start w-fit px-3 py-1 bg-red-600 rounded-full cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-lg">
                                    <div className="flex items-center space-x-1 text-white text-xs font-semibold">
...
export default SubcomponentesTable;
