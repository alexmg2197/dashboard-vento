...
const data = [
  { componente: "Llanta", sub: "Válvula" },
  { componente: "Llanta", sub: "Aro" },
  { componente: "Motor", sub: "Pistón" },
  { componente: "Faro", sub: "Cristal" }
];

const SubcomponentesTable = ({ search, componenteSeleccionado }) => {

    const [page, setPage] = useState(1);
    const rowsPerPage = 5;

    const filtered = data.filter(d =>
        d.componente === componenteSeleccionado &&
  ...
  return (
    <>
         <Box sx={{ mb: 2, display: 'flex', justifyContent: 'flex-end' }}>
            <button className="group flex items-center justify-start w-11 h-11 bg-green-600 rounded-full cursor-pointer relative overflow-hidden transition-all duration-200 shadow-lg hover:w-52 hover:rounded-lg active:translate-x-1 active:translate-y-1">
